"""
Usage:
    python -m starter.render_tetrahedron --image_size 256
"""
import argparse
import numpy as np
import imageio
...
def render_tetrahedron(image_size=256, color=[0.7, 0.7, 1], num_views=36, device=None):
    ...
    vertices = torch.tensor(
        [
            [-0.5, -1/(2*np.sqrt(3)), 0.0],  # base 0
            [0.0, np.sqrt(3)/2 - 1/(2*np.sqrt(3)), 0.0],  # base 1
            [0.5, -1/(2*np.sqrt(3)), 0.0],  # base 2
            [0.0, 0.0, np.sqrt(2/3)],  # apex
        ],
        dtype=torch.float32,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="images/tetrahedron_render.gif")
    parser.add_argument("--image_size", type=int, default=256)
    args = parser.parse_args()
    image_list = render_tetrahedron(image_size=args.image_size)
