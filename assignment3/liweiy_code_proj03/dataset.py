...
def get_nerf_datasets(
    ...
    train_data = torch.load(cameras_path, weights_only=False)
    ...
    ratio = 1
    # Create a mask where indices are divisible by `ratio`
    mask_train = torch.arange(train_idx.shape[0]) % ratio == 0
    mask_val = torch.arange(val_idx.shape[0]) % ratio == 0
    mask_test = torch.arange(test_idx.shape[0]) % ratio == 0

    # Apply the modulus operation only on selected elements
    train_idx = train_idx[mask_train]
    val_idx = val_idx[mask_val]
    test_idx = test_idx[mask_test]
