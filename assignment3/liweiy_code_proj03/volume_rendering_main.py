...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        # print(cam_idx, " ", file_prefix)
        if cam_idx == 0 and file_prefix == '':
            pass
            # plt.imshow(vis_grid(xy_grid, image_size))
            plt.imsave(f'images/xy_grid_{cam_idx}.png',  vis_grid(xy_grid, image_size))
            # plt.show()
        ...
        if cam_idx == 0 and file_prefix == '':
            pass
            render_points(f'images/sample_points_{cam_idx}.png', 
                          ray_bundle.sample_points.view(-1,3).unsqueeze(0), 
                          device=device)
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        if (
            ...
            with torch.no_grad():
                ...
                imageio.mimsave(f'images/{cfg.output_prefix}_part_3.gif', [np.uint8(im * 255) for im in test_images], loop=0)
