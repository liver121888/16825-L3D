...
def get_pixels_from_image(image_size, camera):
    ...
    x = torch.arange(0, W).to(camera.device) + 0.5
    y = torch.arange(0, H).to(camera.device) + 0.5
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    xy_grid_sub = xy_grid[torch.randperm(len(xy_grid))[:n_pixels], :]
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    rays_o = torch.zeros_like(ndc_points[..., -1:]) + camera.get_camera_center()
