...
def setup_optimizer(gaussians):
    ...
    parameters = [
        {'params': [gaussians.pre_act_opacities], 'lr': 5e-2, "name": "opacities"},
        {'params': [gaussians.pre_act_scales], 'lr': 5e-3, "name": "scales"},
        {'params': [gaussians.colours], 'lr': 2e-2, "name": "colours"},
        {'params': [gaussians.means], 'lr': 5e-4, "name": "means"},
    ]
...
def run_training(args):
    ...
    for itr in range(args.num_itrs):
        ...
        pred_img, depth, mask = scene.render(camera, bg_colour=(0.0, 0.0, 0.0), img_size=train_dataset.img_size, per_splat=args.gaussians_per_splat)
    ...
    for test_data in tqdm(test_loader, desc="Running Evaluation"):
        ...
        with torch.no_grad():
            ...
            # HINT: Get per_splat from args.gaussians_per_splat
            # HINT: camera is available above
            pred_img, depth, mask = scene.render(camera, bg_colour=(0.0, 0.0, 0.0), img_size=test_dataset.img_size, per_splat=args.gaussians_per_splat)
